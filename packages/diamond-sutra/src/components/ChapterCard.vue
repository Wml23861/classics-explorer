...
<template>
  <router-link :to="`${linkPrefix || '/diamond'}/${chapter.id}`" class="group block no-underline">
    <div class="card card-hover h-full flex flex-col" style="padding: 1.75rem;">
      ...
      <div class="flex items-start justify-between mb-5">
        <div class="w-10 h-10 rounded-lg flex items-center justify-center text-white font-bold text-base shadow-sm transition-transform duration-300 group-hover:scale-110"
             :style="{ background: badgeColor(chapter.id) }">
          {{ chapter.id }}
        </div>
      ...
      <!-- Title -->
      <h3 class="text-lg font-semibold tracking-[0.12em] mb-3 transition-colors" style="color: #5a4020;">
        {{ chapter.title }}
      </h3>

      <!-- Subtitle -->
      <p class="text-base leading-relaxed tracking-wide mb-5 line-clamp-2 flex-1" style="color: #6b5d4a;">
        {{ chapter.subtitle }}
      </p>

      <!-- Tags -->
      <div class="flex flex-wrap gap-1.5 mb-5">
        <span v-for="tag in chapter.tags?.slice(0, 3)" :key="tag" class="tag">
          {{ tag }}
        </span>
      </div>
      ...
      <div class="flex justify-end">
        <div class="w-7 h-7 rounded-full border flex items-center justify-center transition-all duration-300 group-hover:bg-[#8b6914]/8"
             style="color: #b8a080; border-color: rgba(180, 150, 100, 0.3);">
